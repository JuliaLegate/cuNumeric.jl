...
function recalibrate_allocator!()
    ...
    recal = ccall((:nda_recalibrate_allocator, libnda), Int64, ())
    @assert recal >= 0
    # if recal != current_bytes[]
    #     @info "[cuNumeric GC] Recalibrated allocator: $recal bytes (previously $(current_bytes[]) bytes)"
    # end
