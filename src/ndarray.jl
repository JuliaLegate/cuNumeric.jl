...
Base.eltype(arr::NDArray) = Legate.code_type_map[Int(Legate.code(unsafe_load(nda_array_type(arr))))]
...
function Base.getindex(arr::NDArray, i::UnitRange, j::Colon)
    return nda_get_slice(arr, slice_array(slice(first(i) - 1, last(i)), slice(0, Base.size(arr, 2))))
end

function Base.getindex(arr::NDArray,  i::Colon, j::UnitRange)
    return nda_get_slice(arr, slice_array(slice(0, Base.size(arr, 1)), slice(first(j) - 1, last(j))))
end
...
function Base.getindex(arr::NDArray, i::UnitRange, j::UnitRange)
    return nda_get_slice(arr, slice_array(slice(first(i) - 1, last(i)), slice(first(j) - 1, last(j))))
end
...
function reshape(arr::NDArray, i::Dims{N}) where N
    return nda_reshape_array(arr, UInt64.(collect(dims)))
end
