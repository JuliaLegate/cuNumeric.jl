...
RUN echo "export LD_LIBRARY_PATH=\$(julia -e 'print(Sys.BINDIR * \"/../lib\")'):\$LD_LIBRARY_PATH" >> /etc/.env
RUN chmod +x /etc/.env
RUN cat /etc/.env
...
RUN source /etc/.env && julia --color=yes -e ' \
    using Pkg; \
    Pkg.add(PackageSpec(url = "https://github.com/JuliaLegate/Legate.jl", rev = "driver-compat")) \
'
