...
function check_svd_orthonormality(u, vh, tol_a, tol_r)
    ...
    Vh = Array(vh)
    k  = size(U, 2)
    ok_u  = isapprox(U'  * U,  Matrix{eltype(U)}(I, k, k);  atol=tol_a, rtol=tol_r)
    ok_vh = isapprox(Vh * Vh', Matrix{eltype(Vh)}(I, k, k); atol=tol_a, rtol=tol_r)
    return ok_u && ok_vh
end
...
@testset "svd rank-1 matrix" begin
    @testset verbose=true for T in Base.uniontypes(cuNumeric.SUPPORTED_SVD_TYPES)
        ...
        # 5×4 satisfies the M >= N constraint
        v1    = T.(collect(1:5))
        v2    = T.(collect(1:4))
        A_ref = v1 * v2'
        nda   = cuNumeric.NDArray(A_ref)
        ...
        allowscalar() do
            sv = Array(s)
            @test sv[1] > atol(T)
            @test all(sv[2:end] .< atol(T) * 10)
        end
    end
end

@testset "svd agrees with LinearAlgebra.svd" begin
    @testset verbose=true for T in Base.uniontypes(cuNumeric.SUPPORTED_SVD_TYPES)
        A_ref = my_rand(T, 5, 4)
        nda   = cuNumeric.NDArray(A_ref)
        _, s, _ = cuNumeric.svd(nda, false)
        ref   = LinearAlgebra.svd(A_ref)
        allowscalar() do
            # Singular values must match; signs of singular vectors may differ
            @test cuNumeric.compare(ref.S, s, atol(T), rtol(T))
        end
    end
end

@testset "svd 1d guard" begin
    @testset verbose=true for T in Base.uniontypes(cuNumeric.SUPPORTED_SVD_TYPES)
        v = cuNumeric.NDArray(T.(collect(1:4)))
        @test_throws ArgumentError cuNumeric.svd(v)
    end
end

@testset "svd 3d guard" begin
    @testset verbose=true for T in Base.uniontypes(cuNumeric.SUPPORTED_SVD_TYPES)
        A = cuNumeric.NDArray(my_rand(T, 3, 3, 3))
        @test_throws ArgumentError cuNumeric.svd(A)
    end
end

@testset "svd M < N guard" begin
    @testset verbose=true for T in Base.uniontypes(cuNumeric.SUPPORTED_SVD_TYPES)
        A = cuNumeric.NDArray(my_rand(T, 3, 5))
        @test_throws ArgumentError cuNumeric.svd(A)
    end
end