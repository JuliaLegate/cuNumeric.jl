    ...
    @testset "Binary Ops" begin
        ...
        @testset for T in types
            ...
            allowscalar() do
                ...
                @test ndims(neq) == 0
                @test fetch(eq)
                @test !fetch(arr_cn == arr_cn2)
                @test fetch(neq)
                @test !fetch(arr_cn != arr_cn)
                @test fetch(all(arr_cn .== arr_cn))
            end
        end
    end
end
...
function run_array_equal_tests()
    @testset "0-d == / !=" begin
        ...
        allowscalar() do
            ...
            @test fetch(n32 == n64) == (a32 == a64)
            @test fetch(n32 != n64) == (a32 != a64)

            b64 = Float64[1, 2, 4]
            m64 = NDArray(b64)
            @test fetch(n32 == m64) == (a32 == b64)
            @test fetch(n32 != m64) == (a32 != b64)
        end
        ...
        allowscalar() do
            ...
            @test !fetch(same == other_shape)
            @test fetch(same != other_shape)
            @test !fetch(same == other_rank)
            @test fetch(same != other_rank)
        end
    end
end
