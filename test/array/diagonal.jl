...
function _host_scalar_compare(ref, out::NDScalar, ::Type{T}) where {T}
    allowscalar() do
        @test ref ≈ unwrap(out) atol=atol(T) rtol=rtol(T)
    end
end

function _host_bool_compare(ref::Bool, out::NDReal{Bool})
    allowscalar() do
        @test unwrap(out) == ref
    end
end
