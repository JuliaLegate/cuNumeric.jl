...
function _host_contract_compare(ref, out, ::Type{T}) where {T}
    allowscalar() do
...
@testset "contract GEMM" begin
    @testset for T in (Float32, Float64, ComplexF32, ComplexF64)
        ...
        @test size(C) == (5, 6)
        _host_contract_compare(ref_ij, C, T)

        out = cuNumeric.zeros(T, 5, 6)
        contract!(out, "ij", nda, "ik", ndb, "kj")
        _host_contract_compare(ref_ij, out, T)

        C_int = contract(nda, (1, 2), ndb, (2, 3))
        _host_contract_compare(ref_ij, C_int, T)
        ...
        for (Ause, Am) in ((nda, "ik"), (ndaT, "ki"))
            for (Buse, Bm) in ((ndb, "kj"), (ndbT, "jk"))
                for swap in (false, true)
                    ...
                    _host_contract_compare(ref_ij, Cij, T)
                    Cji = cuNumeric.zeros(T, 6, 5)
                    contract!(Cji, "ji", X, Xm, Y, Ym)
                    _host_contract_compare(ref_ji, Cji, T)
                end
            end
        end
    end
...
@testset "contract MV / VV / outer" begin
    @testset for T in (Float32, Float64)
        ...
        _host_contract_compare(ref_mv, contract(nda, "ij", ndx, "j"), T)
        _host_contract_compare(ref_mv, contract(ndx, "j", nda, "ij"), T)

        @tensor ref_vm[i] := A[j, i] * y[j]
        _host_contract_compare(ref_vm, contract(nda, "ji", ndy, "j"), T)
        _host_contract_compare(ref_vm, contract(ndy, "j", nda, "ji"), T)
        ...
        @test ndims(dot) == 0
        _host_contract_compare(ref_dot, dot, T)
...
@testset "contract batched" begin
    @testset for T in (Float32, Float64)
        ...
        for (cm, p) in (
            ...
            _host_contract_compare(permutedims(ref, p), C, T)
        end
...
@testset "contract alpha/beta" begin
    @testset for T in (Float32, Float64)
        ...
        _host_contract_compare(β3 * seed_ji + α2 * permutedims(prod), out_ji, T)
    end
end
...
@testset "tensordot" begin
    @testset for T in (Float32, Float64)
        ...
        _host_contract_compare(ref, D, T)
    end
end
