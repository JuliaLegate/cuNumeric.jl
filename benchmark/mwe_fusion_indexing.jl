...
function main(args=ARGS)
    ...
    println(
        "Expected last partition start (zero-based, equal tiles): $((p-1)*cld(n,p)); Int32 limit=$(typemax(Int32))"
    )
    x, y = stage("allocate and fill input/output") do
        return cuNumeric.ones(Float32, n), cuNumeric.zeros(Float32, n)
    end
    ...
    stage("$mode square / negate / exp") do
        ...
        tree = Base.Broadcast.instantiate(
            Base.Broadcast.broadcasted(exp, Base.Broadcast.broadcasted(-, square))
        )
        if mode == "fused"
    ...
    stage("verify output markers (no reduction)") do
        for i in points
            ...
            @assert isapprox(actual, expected; rtol=2.0f-5, atol=2.0f-6) "output mismatch at $i: got $actual expected $expected"
        end
    end
    return println("PASS: all $(length(points)) markers; P=$p N=$n mode=$mode")
end
