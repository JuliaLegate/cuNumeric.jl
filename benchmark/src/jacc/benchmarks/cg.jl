...
function cg_matvec(i, lower, diagonal, upper, p, Ap)
    ...
    @inbounds Ap[i] =
        diagonal[i]*p[j] + (j>1 ? lower[i]*p[j - 1] : zero(diagonal[i])) +
...
# JACC.Multi.sync_ghost_elems! stages each complete partition through the host.
# CG only needs the two values adjacent to every partition boundary, so exchange
# those values directly between the device arrays instead.
function cg_exchange_halos!(parts)
    for i in 1:(length(parts) - 1)
        left, right = parts[i], parts[i + 1]
        copyto!(right, 1, left, length(left)-1, 1)
        copyto!(left, length(left), right, 2, 1)
    end
    return nothing
end

function cg_sync_halos!(p)
    length(p.a2)==1 && return nothing
    cg_exchange_halos!(p.a2)
