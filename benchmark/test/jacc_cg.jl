...
@testset "CG partition kernels" begin
    for T in (Float32,Float64), devices in (1,2,3,4)
        n=12; width=n÷devices; p=T.(1:n); Ap=zeros(T,n); dot=zero(T)
        for d in 1:devices
            lo=(d-1)*width+1; hi=d*width
...
@testset "JACC Multi CG solve" begin
    for T in (Float32,Float64), every in (1,4,30), limit in (1,40)
        b=JACCCG{T}(;N=16JACC.Multi.ndev(),gpus=JACC.Multi.ndev(),check_every=every,max_iter=limit)
